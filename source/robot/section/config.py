...
from robot.comm.status import status_dict, ChatStatus, AntiStatus
...
@SplitCommand('anti').trim_super()
async def anti(session: Session, para: str):
    match para:
        case 'on':
            AntiStatus[session.id].switch = True
            await session.reply('anti功能已开启')
        case 'off':
            AntiStatus[session.id].switch = False
            await session.reply('anti功能已关闭')
